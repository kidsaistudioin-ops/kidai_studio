...
export async function callGemini({
  ...
  const maskedKey = process.env.GEMINI_API_KEY.slice(-4);
  console.log(`🔑 Calling Gemini with API Key ending in: ...${maskedKey}`);
  ...
  for (const currentModel of modelsList) {
    try {
      ...
      try {
        ...
        console.log("⚠️ [GEMINI] JSON Parse Failed, returning raw text.");
      ...
      if (errMsg.includes("API_KEY_INVALID") || errMsg.includes("403")) {
        ...
        return { success: false, error: `[GOOGLE CLOUD 404] Key ending in ${maskedKey} is failing. SOLUTION: aistudio.google.com se ek FRESH key banayein, .env.local me dalein, file SAVE karein aur server Restart karein!` };
      }
      else if (errMsg.includes("404") || errMsg.includes("not found")) {
        console.log(`⚠️ MODEL ERROR: Model '${currentModel}' is API key par available nahi hai. Next fallback try kar rahe hain...`);
        continue;
      } 
      else if (errMsg.includes("429") || errMsg.includes("503") || errMsg.includes("Quota") || errMsg.includes("Too Many Requests") || errMsg.includes("high demand")) {

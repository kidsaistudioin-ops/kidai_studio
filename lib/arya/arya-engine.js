...
function buildAryaSystemPrompt(context) {
  const { student, reviewQueue, weakTopics, todaySession, stats } = context
...
- Bor mat hone do, har din alag type ke game se revision karao.
...
export async function recordQuizAttempt(studentId, quizId, selectedOption, timeTaken) {
  ...
  // COIN update karo (+2 for correct, -2 for wrong)
  const coinsEarned = isCorrect ? 2 : -2
...
// ============================================
export async function generateGameFromScan(imageBase64, studentAge = 10, studentMedium = "English", subject = "Mixed", weakTopics = [], favoriteGame = "quiz", preferredGamesList = []) {
  ...
  const prompt = PROMPTS.homework(studentAge, subject, weakTopics, favoriteGame) + extraInstructions;
  ...
  const res = await generateAI({
    ...
    prompt,
    images: [imageBase64],
    preferred: 'gemini' // Image parsing ke liye Gemini sabse sasta/badhiya hai
  });
  ...
  // LAYER 4: SAVING TO DB FOR MANUAL REVIEW (Future / Logic ready)
  // await supabase.from('generated_games').insert({ student_id: ..., data: gameData, status: 'pending' })
